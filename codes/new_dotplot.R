...
common_CNN_RF <- intersect(RF_genes, CNN_genes) #7356
unique_RF <- setdiff(RF_genes, union(real_genes, CNN_genes))#1577 
unique_CNN <- setdiff(CNN_genes, union(real_genes, RF_genes)) #281 c
...
for (disease in top_diseases) {
  ...
  # Calculate p-values for each gene list
  ...
  # Store the -log10(p-values) in the enrichment_data dataframe
  enrichment_data <- rbind(enrichment_data, data.frame(
    Disease = disease, 
    unique_RF = -log10(unique_RF_pval + 1e-10),
    unique_CNN = -log10(unique_CNN_pval + 1e-10),
    real = -log10(real_pval + 1e-10)
  ))
}
...
enrichment_data_melt$List <- factor(enrichment_data_melt$List, 
                                    levels = c("unique_RF", "unique_CNN", "real"))
...
ggplot(enrichment_data_melt, aes(x = List, y = Disease, size = Enrichment, color = Enrichment)) +
  ...
  theme_minimal() +
  labs(title = "Enrichment of Gene Lists",
       x = "Gene List",
       y = "Disease Dataset",
       size = "-log10(p-value)",
       color = "-log10(p-value)") +  # Include the color legend
...
###############################################paper 5 categories adjusted p value##############################################
...
calc_enrichment <- function(gene_list, disease_genes, all_genes) {
  ...
  # return(fisher_result$p.value)
  # Return both p-value and adjusted p-value
  return(list(p_value = fisher_result$p.value,
              adj_p_value = p.adjust(fisher_result$p.value, method = "BH")))
}
...
for (disease in top_diseases) {
  ...
  # Calculate p-values and adj_p_vale for each gene list
  real_pval <- calc_enrichment(real_genes, disease_genes, all_genes)
  unique_RF_pval <- calc_enrichment(unique_RF, disease_genes, all_genes)
  unique_CNN_pval <- calc_enrichment(unique_CNN, disease_genes, all_genes)
  ...
  # Store the -log10(p-values) in the enrichment_data dataframe (here I just pick adj_pvalue)
  enrichment_data <- rbind(enrichment_data, data.frame(
    Disease = disease, 
    unique_RF_adj = -log10(unique_RF_pval$adj_p_value + 1e-10),
    unique_CNN_adj = -log10(unique_CNN_pval$adj_p_value + 1e-10),
    real_adj = -log10(real_pval$adj_p_value + 1e-10)
    , RF_adj = -log10(RF_pval$adj_p_value + 1e-10)
    ,CNN_adj = -log10(CNN_pval$adj_p_value  + 1e-10)
  ))
}
...
# Specify the desired order of the List factor
enrichment_data_melt$List <- factor(enrichment_data_melt$List, 
                                    levels = c("unique_RF_adj", "unique_CNN_adj", "real_adj", "RF_adj", "CNN_adj"))
...
ggplot(enrichment_data_melt, aes(x = List, y = Disease, size = Enrichment, color = Enrichment)) +
  ...
  scale_color_gradient(low = "lightgreen", high = "darkgreen") +  # Apply gradient shades of green
  theme_minimal() +
  labs(title = "Enrichment of Gene Lists adj- p-value",
       x = "Gene List",
       y = "Disease Dataset",
       size = "-log10(adj-p-value)",
       color = "-log10(adj-p-value)") +  # Include the color legend
  theme(axis.text.y = element_text(size = 8),  # Adjust text size for better readability
        axis.text.x = element_text(size = 10, angle = 45, hjust = 1))  # Rotate x-axis labels for better readability



#############################paper 3 categories adjusted p value (manuscript) ##############################################
